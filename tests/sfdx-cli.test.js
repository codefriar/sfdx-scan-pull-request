...
describe("CLI tests!", () => {
    it("reports pmd violations successfully", async () => {
        ...
        expect(scannerViolations.find((violation) => violation.ruleName === "ApexUnitTestClassShouldHaveAsserts")).toBeTruthy();
        expect(scannerViolations.find((violation) => violation.ruleName === "EmptyStatementBlock")).toBeTruthy();
    ...
    it("reports eslint violations successully", async () => {
        ...
        expect(findings[0].violations.filter((violation) => violation.ruleName === "no-console")).toBeTruthy();
    }, thirtySecondsRunTime);
...
//# sourceMappingURL=sfdx-cli.test.js.map