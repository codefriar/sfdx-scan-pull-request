...
export class CommentsReporter extends BaseReporter<GithubComment> {
  ...
  private async createOneReviewWithMultipleComments(comments: GithubComment[]) {
    ...
    const validComments = comments.filter((comment) => {
      ...
      if (!lineInDiff) {
        this.logger(
          `WARNING: Rejecting comment for ${comment.path} line ${comment.line} - not in diff. Changed lines: ${Array.from(diffInfo).sort((a, b) => a - b).join(', ')}`
        );
        return false;
      }
    ...
    const githubReviewComments: GithubReviewComment[] = validComments.map(
      (comment) => {
        ...
        if (isMultiLine && diffInfo) {
          ...
          this.logger(
            `  Multi-line: start_line ${comment.start_line} in diff: ${startLineInDiff}, end line ${comment.line} in diff: ${endLineInDiff}`
          );
